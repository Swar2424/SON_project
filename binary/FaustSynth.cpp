...
class mydsp : public dsp {
 ...
 public:
	...
	virtual void buildUserInterface(UI* ui_interface) {
		...
		ui_interface->addNumEntry("curdel", &fEntry3, FAUSTFLOAT(5.0f), FAUSTFLOAT(1.0f), FAUSTFLOAT(1e+01f), FAUSTFLOAT(0.01f));
		ui_interface->addNumEntry("cutoff", &fEntry0, FAUSTFLOAT(1e+04f), FAUSTFLOAT(5e+01f), FAUSTFLOAT(1e+04f), FAUSTFLOAT(0.01f));
		ui_interface->addNumEntry("freq", &fEntry2, FAUSTFLOAT(2e+02f), FAUSTFLOAT(5e+01f), FAUSTFLOAT(1e+03f), FAUSTFLOAT(0.01f));
	...
	virtual void compute(int count, FAUSTFLOAT** RESTRICT inputs, FAUSTFLOAT** RESTRICT outputs) {
		...
		float fSlow4 = std::max<float>(fSlow3, 23.44895f);
		float fSlow5 = std::max<float>(2e+01f, std::fabs(fSlow4));
		float fSlow6 = fConst4 * fSlow5;
		float fSlow7 = fConst5 / fSlow5;
		float fSlow8 = std::max<float>(0.0f, std::min<float>(2047.0f, fConst6 / fSlow4));
		int iSlow9 = int(fSlow8);
		int iSlow10 = iSlow9 + 1;
		float fSlow11 = std::floor(fSlow8);
		float fSlow12 = fSlow8 - fSlow11;
		float fSlow13 = fSlow11 + (1.0f - fSlow8);
		float fSlow14 = fConst7 * fSlow3;
		float fSlow15 = fConst1 * float(fEntry3);
		for (int i0 = 0; i0 < count; i0 = i0 + 1) {
			...
			float fTemp3 = ((1 - iVec0[1]) ? 0.0f : fSlow6 + fRec6[1]);
			fRec6[0] = fTemp3 - std::floor(fTemp3);
			float fTemp4 = mydsp_faustpower2_f(2.0f * fRec6[0] + -1.0f);
			fVec1[0] = fTemp4;
			float fTemp5 = fSlow7 * float(iVec0[1]) * (fTemp4 - fVec1[1]);
			fVec2[IOTA0 & 2047] = fTemp5;
			fRec5[0] = 0.999f * fRec5[1] + fTemp5 - (fSlow13 * fVec2[(IOTA0 - iSlow9) & 2047] + fSlow12 * fVec2[(IOTA0 - iSlow10) & 2047]);
			float fTemp6 = fSlow14 * fRec5[0] * fRec4[0] * fRec3[0];
			fVec3[0] = fTemp6;
			fRec2[0] = -((fRec2[1] * (1.0f - fTemp1) - (fTemp6 + fVec3[1])) / (fTemp1 + 1.0f));
			fRec1[0] = fRec2[0] - (fRec1[2] * ((fTemp1 + -1.0f) / fTemp0 + 1.0f) + 2.0f * fRec1[1] * (1.0f - 1.0f / mydsp_faustpower2_f(fTemp0))) / fTemp2;
			float fTemp7 = (fRec1[2] + fRec1[0] + 2.0f * fRec1[1]) / fTemp2;
			fVec4[0] = fTemp7;
			fRec8[0] = fSlow15 + fConst2 * fRec8[1];
			int iTemp8 = int(fRec8[0]);
			float fTemp9 = std::floor(fRec8[0]);
			float fRec7 = -(fVec4[std::min<int>(11, std::max<int>(0, iTemp8))] * (fTemp9 + (1.0f - fRec8[0])) + (fRec8[0] - fTemp9) * fVec4[std::min<int>(11, std::max<int>(0, iTemp8 + 1))]);
			float fTemp10 = 0.5f * (fRec7 + fTemp7);
	...
	#define FAUST_FILE_NAME "FaustSynth.dsp"
	...
	FAUST_ADDNUMENTRY("curdel", fEntry3, 5.0f, 1.0f, 1e+01f, 0.01f);
	FAUST_ADDNUMENTRY("cutoff", fEntry0, 1e+04f, 5e+01f, 1e+04f, 0.01f);
	FAUST_ADDNUMENTRY("freq", fEntry2, 2e+02f, 5e+01f, 1e+03f, 0.01f);
	FAUST_ADDNUMENTRY("gain", fEntry1, 0.5f, 0.0f, 1.0f, 0.01f);
	FAUST_ADDBUTTON("gate", fButton0);

	#define FAUST_LIST_ACTIVES(p) \
		p(NUMENTRY, curdel, "curdel", fEntry3, 5.0f, 1.0f, 1e+01f, 0.01f) \
		p(NUMENTRY, cutoff, "cutoff", fEntry0, 1e+04f, 5e+01f, 1e+04f, 0.01f) \
		p(NUMENTRY, freq, "freq", fEntry2, 2e+02f, 5e+01f, 1e+03f, 0.01f) \
...
FaustSynth::FaustSynth() : AudioStream(FAUST_INPUTS, new audio_block_t*[FAUST_INPUTS])
{
#ifdef NVOICES
    int nvoices = NVOICES;
...
void FaustSynth::updateImp(void)
{
#if MIDICTRL
...
void FaustSynth::update(void) { updateImp<FAUST_INPUTS, FAUST_OUTPUTS>(); }

void FaustSynth::setParamValue(const std::string& path, float value)
{
    fUI->setParamValue(path, value);
}

float FaustSynth::getParamValue(const std::string& path)
{
